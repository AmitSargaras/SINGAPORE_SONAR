...
//					}
			...
			if (null != refArr) {
				for (int i = 0; i < refArr.length; i++) {
					...
					if("BAHRAIN".equals(xref.getFacilitySystem()) || "HONGKONG".equals(xref.getFacilitySystem()) 
							|| "GIFTCITY".equals(xref.getFacilitySystem())
							|| singaporeFacSystem.equals(xref.getFacilitySystem())) {
		...
		if(lineIdNotToSend.size()>0){
			...
			if (null != refArr) {
				for (int i = 0; i < refArr.length; i++) {
					...
					ResourceBundle bundle = ResourceBundle.getBundle("ofa");
					String singaporeFacSystem =bundle.getString("singapore.systemName");
					if(singaporeFacSystem.equals(xref.getFacilitySystem())) {
						xref.setLiabBranch("401");
					}
					xref.setUdfDelete("");
